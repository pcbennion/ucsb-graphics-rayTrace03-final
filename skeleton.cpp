...
void Skeleton::RotateBoneDirToLocalCoordSystem()
{
  for(int i=1; i<NUM_BONES_IN_ASF_FILE; i++) 
  {
    ...
    MATRIX4 Rz = MATRIX4::RotationZ(-m_pBoneList[i].axis_z * M_PI / 180.0);
    MATRIX4 Ry = MATRIX4::RotationY(-m_pBoneList[i].axis_y * M_PI / 180.0);
    MATRIX4 Rx = MATRIX4::RotationX(-m_pBoneList[i].axis_x * M_PI / 180.0);
